...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div 
        className="relative w-full max-w-2xl rounded-2xl bg-[#1A1A1A] border border-[#2D2D2D] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-5 border-b border-[#2D2D2D] flex items-center justify-between bg-[#0F0F0F]/80">
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              />
              <span className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-emerald-500 border-2 border-[#2D2D2D] rounded-full" />
            </div>

            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-bold text-[#FFFFFF]">{user.name}</h2>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2.5 py-1.5 rounded-xl bg-[#1A1A1A] hover:bg-[#2D2D2D] text-[#FFFFFF] text-xs font-medium flex items-center gap-1.5 transition-colors"
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-[#2D2D2D] bg-[#0F0F0F]/40 px-4 text-xs font-medium">
          <button
        ...
        <div className="p-5 overflow-y-auto space-y-4 flex-1">
          ...
          {activeTab === 'profile' && (
            <form onSubmit={handleSaveProfile} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-[#A0A0A0] mb-1.5">Avatar</label>
                  ...
                  {PRESET_AVATARS.map((url, i) => (
                    <button
                      ...
                      className={`relative w-12 h-12 rounded-xl overflow-hidden shrink-0 border-2 transition-all ${
                        avatar === url ? 'border-emerald-500 scale-105 shadow-md' : 'border-[#2D2D2D] opacity-70 hover:opacity-100'
                      }`}
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-[#A0A0A0] mb-1">Display Name</label>
                  <input
                    type="text"
                    required
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    className="w-full px-3 py-2 bg-[#0F0F0F] border border-[#2D2D2D] rounded-xl text-xs text-[#FFFFFF] focus:outline-none focus:border-emerald-500/60"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled
                    value={user.email}
                    className="w-full px-3 py-2 bg-[#0F0F0F]/50 border border-[#2D2D2D]/80 rounded-xl text-xs text-[#A0A0A0] cursor-not-allowed"
                  />
                </div>

                <div>
                  <label className="block text-xs font-medium text-[#A0A0A0] mb-1">Job Title / Role</label>
                  <input
                    type="text"
                    value={role}
                ...
                <div>
                  <label className="block text-xs font-medium text-[#A0A0A0] mb-1">Organization / Team</label>
                  <input
                    type="text"
                    value={company}
          ...
          {activeTab === 'preferences' && (
            <div className="space-y-4">
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-left transition-all flex items-center justify-between ${
                        defaultVoice === voice
                          ...
                          : 'bg-[#0F0F0F] border-[#2D2D2D] text-[#A0A0A0] hover:text-[#FFFFFF] hover:bg-[#1A1A1A]/50'
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <Volume2 className={`w-3.5 h-3.5 ${defaultVoice === voice ? 'text-emerald-400' : 'text-[#A0A0A0]'}`} />
              ...
              <div className="space-y-3 pt-2">
                <label className="flex items-center justify-between p-3 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D] cursor-pointer">
                  <div>
                    <div className="text-xs font-medium text-[#FFFFFF]">Auto Audio Readout</div>
                  ...
                  <input
                    ...
                    className="w-4 h-4 rounded border-[#2D2D2D] bg-[#1A1A1A] text-emerald-500 focus:ring-0"
                  />
                </label>

                <label className="flex items-center justify-between p-3 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D] cursor-pointer">
                  <div>
                    <div className="text-xs font-medium text-[#FFFFFF]">Stream Responses</div>
                    <div className="text-[11px] text-[#A0A0A0]">
          ...
          {activeTab === 'usage' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div className="p-3 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D]">
                  <div className="text-[10px] text-[#A0A0A0] font-mono uppercase">Messages Sent</div>
                ...
                <div className="p-3 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D]">
                  <div className="text-[10px] text-[#A0A0A0] font-mono uppercase">Voice Notes</div>
                ...
                <div className="p-3 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D]">
                  <div className="text-[10px] text-[#A0A0A0] font-mono uppercase">Web Grounding</div>
                  <div className="text-xl font-bold text-[#A3FF12] mt-1 font-mono">
                    {user.stats?.webSearchesPerformed || 0}
                  </div>
                </div>

                <div className="p-3 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D]">
                  <div className="text-[10px] text-[#A0A0A0] font-mono uppercase">Maps Researched</div>
                ...
                <div className="p-3 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D]">
                  <div className="text-[10px] text-[#A0A0A0] font-mono uppercase">Storage Read</div>
                  <div className="text-xl font-bold text-[#A0A0A0] mt-1 font-mono">
              ...
              <div className="p-3.5 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D] space-y-2.5">
                <div className="text-xs font-semibold text-[#FFFFFF]">Connected Cloud Identity Providers</div>
                
                <div className="space-y-2 text-xs">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-[#1A1A1A]/60 border border-[#2D2D2D]/80">
                  ...
                  <div className="flex items-center justify-between p-2 rounded-lg bg-[#1A1A1A]/60 border border-[#2D2D2D]/80">
                    <div className="flex items-center gap-2 text-[#A0A0A0]">
                      <Github className="w-4 h-4 text-[#A0A0A0]" />
          ...
          {activeTab === 'security' && (
            <div className="space-y-4">
              <div className="p-3.5 rounded-xl bg-[#0F0F0F] border border-[#2D2D2D] space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs font-semibold text-[#FFFFFF]">Session Security</div>
                    <div className="text-[11px] text-[#A0A0A0]">Authenticated via token authorization</div>
                  </div>
                ...
                <div className="text-[11px] font-mono p-2 rounded-lg bg-[#1A1A1A] border border-[#2D2D2D] text-[#A0A0A0] break-all">
                ...
                {confirmLogout ? (
                  <div className="flex items-center gap-2 pt-2">
                    ...
                    <button
                      ...
                      className="px-3 py-2 rounded-xl bg-[#1A1A1A] hover:bg-[#2D2D2D] text-[#A0A0A0] text-xs transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
