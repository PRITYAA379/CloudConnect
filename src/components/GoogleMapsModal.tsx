...
export const GoogleMapsModal: React.FC<GoogleMapsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-[#0F0F0F]/80 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className={`w-full bg-[#1A1A1A] border border-[#2D2D2D] rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 ${
          isExpanded ? 'max-w-6xl h-[92vh]' : 'max-w-4xl max-h-[88vh]'
        }`}
      >
        {/* Header */}
        <div className="px-5 py-4 border-b border-[#2D2D2D] bg-[#0F0F0F]/70 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="p-2 rounded-xl text-[#A0A0A0] hover:text-[#FFFFFF] hover:bg-[#1A1A1A] transition-colors hidden sm:flex"
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-xl text-[#A0A0A0] hover:text-[#FFFFFF] hover:bg-[#1A1A1A] transition-colors"
              title="Close modal"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col md:flex-row">
          ...
          <div className="flex-1 min-h-[300px] md:min-h-[420px] bg-[#0F0F0F] relative">
            <iframe
            ...
            {/* Quick overlay badge */}
            <div className="absolute top-3 left-3 px-3 py-1.5 rounded-xl bg-[#0F0F0F]/80 border border-[#2D2D2D] backdrop-blur-md text-[11px] font-mono text-[#A0A0A0] flex items-center gap-2 shadow-lg">
          ...
          {/* Details Sidebar */}
          <div className="w-full md:w-80 p-5 bg-[#0F0F0F]/60 border-t md:border-t-0 md:border-l border-[#2D2D2D] space-y-4 shrink-0 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between gap-2">
                {typeof business.rating === 'number' ? (
                  ...
                  <span className="text-xs text-[#A0A0A0]">Google Maps Place</span>
                )}
              ...
              {/* Address with copy */}
              {(business.formattedAddress || business.address) && (
                <div className="p-3 rounded-xl bg-[#1A1A1A]/60 border border-[#2D2D2D] space-y-1.5 text-xs">
                  <div className="text-[10px] uppercase font-mono text-[#A0A0A0]">Verified Address</div>
                  <div className="text-[#FFFFFF] leading-relaxed">
                    {business.formattedAddress || business.address}
                  </div>
                  <button
                    onClick={handleCopy}
                    className="mt-1 text-[11px] text-[#A3FF12] hover:text-sky-300 flex items-center gap-1 transition-colors"
                  >
                    {copied ? (
                      <>
              ...
              {business.editorialSummary && (
                <div className="text-xs text-[#A0A0A0] leading-relaxed bg-[#1A1A1A]/40 p-3 rounded-xl border border-[#2D2D2D]/60">
                  {business.editorialSummary}
                </div>
              )}
              ...
              {business.openingHours && business.openingHours.length > 0 && (
                <div className="p-3 rounded-xl bg-[#1A1A1A]/50 border border-[#2D2D2D] space-y-1 text-xs text-[#A0A0A0]">
                  <div className="text-[10px] uppercase font-mono text-[#A0A0A0] flex items-center gap-1">
                    <Clock className="w-3 h-3" />
                    <span>Operating Schedule</span>
                  </div>
                  <div className="space-y-1 pt-1 font-mono text-[11px]">
                    {business.openingHours.slice(0, 4).map((h, i) => (
                      <div key={i} className="text-[#A0A0A0] truncate">{h}</div>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-2 pt-4 border-t border-[#2D2D2D]">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-2 px-4 rounded-xl bg-[#1A1A1A] hover:bg-[#2D2D2D] text-[#FFFFFF] text-xs font-medium flex items-center justify-center gap-2 transition-colors"
              >
              ...
              <div className="pt-2 text-[10px] text-[#A0A0A0] leading-normal space-y-1">
                <div className="flex items-start gap-1">
                  <Info className="w-3 h-3 text-[#A0A0A0] shrink-0 mt-0.5" />
                  <span>
                    Grounded with Google Maps Platform API. Data attributed under ID <code className="text-[#A0A0A0] font-mono">gmp_mcp_codeassist_v1_aistudio</code>.
                  </span>
                </div>
                <div className="text-[#A0A0A0] text-[9px]">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[#A0A0A0] hover:text-[#A0A0A0] underline"
                  >
